...
'''
import random as r  # would be at the top of the code
rabNum = 1
rabs = rabNum
for i in range(5):
  for i in range(12):
    for x in range(rabNum):
      rabs += r.randint(1,14)
  rabNum += rabs//2
print(rabs)
...
for i in range(5):
  rabAv = (14+1)/2
  Rabtotal += (rabMom*rabAv)*12
  rabMom += rabAv/6
print(Rabtotal)
 
''' 22. 
   Write a loop which prints the numbers 1 to 110, 11 numbers per line. The program shall print "Coza" in place of the numbers which are multiples of 3, 
   "Loza" for multiples of 5, "Woza" for multiples of 7, "CozaLoza" for multiples of 3 and 5, and so on. Sample output:
